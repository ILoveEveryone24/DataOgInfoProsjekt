...
use web_sys::{HtmlInputElement, DataTransfer, Element, HtmlImageElement, EventSource, MessageEvent};
...
impl Component for Clicker
{
    ...
    fn update(&mut self, _ctx: &Context<Self>, msg: Self::Message) -> bool
    {
        ...
        match msg
        {
            ...
            enumClick::RequestResponse(fetched_data) =>
            {
                // for (key, value) in fetched_data.pieces
                // {
                //     log!("Heres the data: {}", key);
                // }
                
                // self.currentId = fetched_data.moveId;
                // log!("{}", fetched_data.moveId);
                //self.firstVisit = false;
                //self.pieces = fetched_data.pieces;
    ...
    fn view(&self, ctx: &Context<Self>) -> Html{
        ...
        let startX = -100;
        let startY = -100;
        let tmpBool = self.firstVisit.clone();
        let tmpSelf = self.pieces.clone();

        
        let callback = link.callback(|fetched_data| enumClick::RequestResponse(fetched_data));
        

        if(self.madeMove)
        {
            let mut ws = WebSocket::open("ws://127.0.0.1:8000").unwrap();
            ...
            spawn_local(async move {
                while let Some(msg) = read.next().await {
                    ...
                    callback.emit(testingCopy{firstVisit:true, pieces:HashMap::new()});
                }
        ...
        //     spawn_local(async move {
        //         log!("Send a message");
        //         write.send(Message::Text(serde_json::to_string(&testingCopy{firstVisit: tmpBool, pieces: tmpSelf}).unwrap())).await.unwrap();
        //     });
            
        // }
        // spawn_local(async move {
        //         while let Some(msg) = read.next().await {
        //             log!("Take in a message");
        //             let msgCopy = match msg.unwrap()
        //             {
        //                 Message::Text(x) => x,
        //                 Message::Bytes(y) => String::from_utf8_lossy(&y).into_owned(),
        //             };

        //             let returnedVal: testingCopy = serde_json::de::from_str(&msgCopy).unwrap();
        //             for (key, value) in &returnedVal.pieces
        //             {
        //                 //log!("key: {}, value x: {}, value y: {}", key, value.0, value.1);
        //             }
                    

        //             callback.emit(returnedVal);
        //             //log!(format!("1. {:?}", msg))
        //         }
        //         log!("WebSocket Closed")
        //     });
        ...
        //     let callback = link.callback(|fetched_data| enumClick::RequestResponse(fetched_data));
        //     let tmpId = self.currentId.clone();
        //     spawn_local(async move {
        //         let n = 0;
        //         let fetched_data:testingCopy = loop {
        //             let fetched_data:testingCopy = Request::get("/data")
        //             .send()
        //             .await
        //             .unwrap()
        //             .json()
        ...
        //             if tmpId != fetched_data.moveId
        //             {
        //                 break fetched_data;
        //             }
        //     };
        //         callback.emit(fetched_data);
        ...
        //     spawn_local(async move{
        //         let fetched_data:testingCopy = loop{
        //             let temp_data:testingCopy = match Request::get("/data")
        //             .send()
        //             .await
        //             .unwrap()
        //             .json()
        ...
        //                 Err(err) =>
        //                 {
        //                     testingCopy{moveId: 0, pieces:HashMap::new()}
        //                 }
        //             };
        //         };
            ...
            {self.pieces.iter().map
                (|(key, piece)| html!{<img src={(piece.2).to_string()} 
                    ...
                    style = {
                        ...
                        {
                            format!("top:{}px; left:{}px; position:absolute;", (piece.1).to_string(), (piece.0 + 332).to_string())
                        }
                    }
